...
def test_exact():
    ...
    lesses = exact_py(a_values, b_values, c_values, d_values, "less")
    np.testing.assert_array_almost_equal(lesses, np.array([0.916666, 0.996303]))

    greaters = exact_py(a_values, b_values, c_values, d_values, "greater")
    np.testing.assert_array_almost_equal(greaters, np.array([0.583333, 0.039707]))

    two_tails = exact_py(a_values, b_values, c_values, d_values, "two-sided")
